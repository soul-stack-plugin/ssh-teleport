...
Supplies SSH credentials to `keeper.push` from Teleport-issued certificates.

A Soul Stack **SshProvider** plugin, built as `soul-ssh-teleport` and launched by the host over
...
make build      # dist/soul-ssh-teleport
```
...
It lived in `soul-stack/examples/module/soul-ssh-teleport` until NIM-825. [ADR-011][adr11]
